package com.jleung.breadcrumb.device_services;
...
public class NfcService {
    ...
    public String readTagFromIntent(Intent intent) throws NfcException {
        ...
        if (action == null) {
            Log.d(TAG, "Received intent null action.");
            return null;
        }
        ...
        switch(action) {
            case NfcAdapter.ACTION_NDEF_DISCOVERED:
                if(intent.getType() != null && intent.getType().equals(MimeType.TEXT_PLAIN)) {
                    return readParcelableTag(intent.getParcelableExtra(NfcAdapter.EXTRA_TAG));
                }
                else {
